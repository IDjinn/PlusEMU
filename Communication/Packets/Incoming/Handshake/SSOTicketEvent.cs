...
public class SsoTicketEvent : IPacketEvent
{
    ...
    public async Task Parse(GameClient session, ClientPacket packet)
    {
        ...
        if (error == null)
        {
            ...
            session.GetHabbo().Look = PlusEnvironment.GetFigureManager().ValidateFigure(session.GetHabbo().Look, session.GetHabbo().VipRank, ClothingGenderExtensions.ParseFromString(session.GetHabbo().Gender), session.GetHabbo().GetClothing().GetClothingParts);
